  ...
  return (
    <section className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 xl:py-24">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 xl:gap-20 items-center min-h-screen">
          
          {/* Left Side - Main Content */}
          <div className="space-y-6 sm:space-y-8 lg:space-y-10 xl:space-y-12 order-2 lg:order-1">
            {/* Badge */}
            <div className={`inline-flex items-center gap-2 sm:gap-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 sm:px-6 py-2 sm:py-3 text-white/90 text-xs sm:text-sm font-medium animate-slide-up ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="space-y-3 sm:space-y-4 lg:space-y-6">
              <h1 className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl font-black leading-tight sm:leading-none animate-slide-up animate-delay-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            {/* Description */}
            <p className={`text-white/80 text-base sm:text-lg md:text-xl lg:text-2xl leading-relaxed max-w-2xl animate-slide-up animate-delay-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className={`flex flex-col sm:flex-row gap-3 sm:gap-4 animate-slide-up animate-delay-400 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
              <button className="group relative bg-white text-emerald-900 px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl font-bold text-base sm:text-lg shadow-2xl transition-all duration-300 transform hover:scale-105 hover:shadow-white/25 active:scale-95">
              ...
              <button className="group relative bg-transparent border-2 border-white/30 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl font-bold text-base sm:text-lg backdrop-blur-sm transition-all duration-300 transform hover:scale-105 hover:border-white/60 hover:bg-white/10 active:scale-95">
          ...
          <div className={`order-1 lg:order-2 animate-slide-up animate-delay-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6">
              {stats.map((stat, index) => (
                <div 
                  key={stat.id} 
                  className="group relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-lg sm:rounded-xl p-3 sm:p-4 lg:p-6 transition-all duration-500 transform hover:scale-105 hover:bg-white/10 hover:border-white/20 hover:shadow-2xl hover:shadow-white/10"
                  style={{
                    animationDelay: `${index * 150}ms`
                  }}
                >
                  {/* Corner Decoration */}
                  <div className="absolute top-3 sm:top-4 right-3 sm:right-4 w-6 h-6 sm:w-8 sm:h-8 border-t-2 border-r-2 border-emerald-400/50 rounded-tr-lg"></div>
                  
                  <div className="space-y-2 sm:space-y-3">
                    <div className="flex flex-col md:flex-row md:flex-wrap items-center justify-center gap-1 md:gap-2 text-center">
                      <span className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-black bg-gradient-to-r from-emerald-300 via-teal-300 to-blue-300 bg-clip-text text-transparent break-words break-all max-w-full">
                        <AnimatedCounter 
                          end={stat.value} 
                          duration={2.5} 
                        />
                      </span>
                      <span className="text-white/60 text-sm sm:text-base md:text-lg lg:text-xl ml-0 md:mr-1">{stat.unit}</span>
                    </div>
